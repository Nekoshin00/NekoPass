...
class FormEditarPassword(tk.Toplevel):

    def __init__(self, parent, Id, Id_pagina, Sourse, Id_user) -> None:
        super().__init__()
        self.parent = parent
        self.id = Id
    ...
    def contruirWidget(self):
        ...
        #Botones
        self.btn_agregar = tk.Button(self, text="Guardar", width=10)
        self.btn_agregar.config(command=self.editar_password)
    ...
    def obtener_datos(self):
        if self.sourse == "pagina":
            ...
            if pagina:
                datos = self.password_sql.obtener_password(self.id)
                datos_nombre_pagina = datos.pagina.nombre_pagina if datos.pagina else "N/A"
            ...
            self.cbx_pagina.set(datos_nombre_pagina)

        self.text_titulo.insert(0, datos.title)
        self.text_usuario.insert(0, datos.username)
        self.text_pass.insert(0, datos.password)
        self.text_url.insert(0, datos.url)
        self.text_notes.insert(0, datos.notes)
    ...
    def editar_password(self):
        respuesta = messagebox.askyesno("Confirmar edicion", "¿Estás seguro de que deseas editar esta contraseña?")
        if respuesta:
            ...
            self.password_sql.modificar_pass(self.id_user, pagina_index, titulo, username, password, url, notes, self.id)
